    ...
    lapply(FUN = function(list_of_fits){
      list_of_fits %>% 
        ...
        (function(resdf) {c(mean(resdf$HC), 
                        ...
                        MC_error(resdf$HC),
                        MC_error(abs(resdf$HC-reference_quantile)),
                        MC_error(resdf$CI_length))} ) %>% 
        t %>% 
        data.frame() %>% 
        setNames(c('Mean_estimate','infCI','med','supCI', 'std', 'Mean_absolute_bias', 'Mean_bias', 'Reference', 'Mean_CI_length','len_infCI','len_med','len_supCI', "MC_error_HC", "MC_error_Mean_absolute_bias", "MC_error_CI_length")) %>% 
        mutate(type = list_of_fits[[1]]$qmethod)
          }) %>% 
        Reduce(rbind,.)
}
  ...
  list_of_list_of_fits %>% 
    lapply(FUN = function(list_of_fits){
      list_of_fits %>% 
        ...
        unlist() %>% 
        (function(x) c(mean(x), quantile(x, probs = c(0.025,0.5,0.975)), Sd(x), MC_error(x)) ) %>% 
        t %>% 
        data.frame() %>% 
        setNames(c('MISE','infCI','med','supCI', 'std', "MC_error")) %>% 
        mutate(type = list_of_fits[[1]]$qmethod)
          }) %>% 
        Reduce(rbind,.)
}
...
expand.grid(c('normal', 'student', 'bimodal'), c(10,20,50,100)) %>% 
...
datasets = expand.grid(c('normal', 'student', 'bimodal'), c(10,20,50,100))
reference_densities = list('normal' = dnorm,
                  'student' = function(x) dt(x, df = 3, ncp = -2),
                  'bimodal' = function(x) dmixnorm(x, mus = c(-2, 5), sigmas = c(1,1), probs = c(2,4) %>% normalise_weights))

reference_quantiles = list('normal' = qnorm(0.05),
                  'student' = qt(0.05, df = 3, ncp = -2),
                  'bimodal' = qmixnorm(0.05, mus = c(-2, 5), sigmas = c(1,1), probs = c(2,4) %>% normalise_weights))
...
for( i in seq_len(nrow(datasets))){
  ...
  to_plot[[i]] = paste(datasets[i,1], datasets[i,2], sep = '') %>% 
    ...
    (function(fit_list){
      df_RMISE = fit_list %>% 
        get_MISE_for_all_methods(reference_dist = reference_densities[[datasets[i,1]]], numit = 2000, ncores = 2) %>% 
        dplyr::select(type, MISE, MC_error) %>% 
        dplyr::rename(MC_error_MISE = MC_error) %>%
        dplyr::mutate(size = datasets[i,2], data_type = datasets[i,1]) 
      
      df_quantile = fit_list %>% 
        get_quantile_CI_mean_length_for_all_methods(reference_quantile = reference_quantiles[[datasets[i,1]]], 
                                                    numit = 10000, ncores = 2) %>% 
        dplyr::select(Mean_absolute_bias, Mean_CI_length, type, MC_error_Mean_absolute_bias, MC_error_CI_length) 
...
to_plot <- to_plot %>%
  ...
  (function(df) {
    estimates <- df %>%
      dplyr::select(-starts_with("MC_error")) %>%
      gather(variable, value, MISE, Mean_absolute_bias:Mean_CI_length)

    MC_errors <- df %>%
      dplyr::select(-MISE, -Mean_absolute_bias, -Mean_CI_length) %>%
      gather(variable, MC_error, starts_with("MC_error")) %>%
      mutate(variable = gsub("MC_error_", "", variable)) %>% 
      mutate(variable = gsub("CI_length", "Mean_CI_length", variable)) 

    left_join(estimates, MC_errors)
  })
...
p = to_plot %>% 
  ...
  mutate(data_type = factor(data_type, 
                            levels = c("Normal", "t-Student", "Bimodal"))) %>% 
  ...
  geom_segment(aes(xend = factor(size), y = value - MC_error, yend = value + MC_error)) +
  xlab('dataset size') +
...
datasets = expand.grid(c('normal', 'student', 'bimodal', 'trimodal'), c(10,20,50,100))
reference_densities = list('normal' = dnorm,
                  'student' = function(x) dt(x, df = 3, ncp = -2),
                  'bimodal' = function(x) dmixnorm(x, mus = c(-2, 5), sigmas = c(1,1), probs = c(2,4) %>% normalise_weights),
                  'trimodal' = function(x) dmixnorm(x, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights)
                  )

reference_quantiles = list('normal' = qnorm(0.05),
                  'student' = qt(0.05, df = 3, ncp = -2),
                  'bimodal' = qmixnorm(0.05, mus = c(-2, 5), sigmas = c(1,1), probs = c(2,4) %>% normalise_weights),
                  'trimodal' = qmixnorm(0.05, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights)
                  )
