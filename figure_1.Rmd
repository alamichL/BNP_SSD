...
create_funlist <- function(nit) {
  list(
    "normal_ssd" = fit_normal_ssd_on_log_data,
...
rmixnorm <- function(n, mus, sigmas, probs) {
  ...
  rmultinom(n = 1, size = n, prob = probs) %>%
    as.numeric() %>%
    (function(x) {
      mapply(FUN = function(n_, mean_, sd_) {
        rnorm(n_, mean_, sd_)
      }, x, mus, sigmas)
    }) %>%
    unlist() %>%
...
create_dataset_list <- function() {
  dataset_creators_list <- list(
    "normal" = create_normal_datasets,
    "student" = create_student_datasets,
    "bimodal" = function(size, number) {
    ...
    "trimodal" = function(size, number) {
      create_multimodal_datasets(
        size = size, number = number,
        mus = c(-5, 0, 5),
        sigmas = c(1, 1, 2),
        unnormalized_weights = c(2, 3, 2)
      )
    }
  )
  ...
  expand.grid(dataset_creators_list %>% names(), sizes) %>%
    (function(grid) {
      mapply(FUN = function(creator_name, size) {
        dataset_creators_list[[creator_name]](size = size, number = 100)
      }, grid$Var1, grid$Var2, SIMPLIFY = F) %>%
...
load_or_compute <- function(dataset_type, idx, method) {
  if (method %in% c("normal_ssd", "logis_ssd", "kde_ssd")) {
    compute(dataset_type, idx, method)
  } else {
    fname <- make_fname(dataset_type, idx, method)
    if (file.exists(fname)) {
      readRDS(fname)
    } else {
      compute_and_save_and_return(dataset_type, idx, method)
    }
  }
}
...
save_if_not_present <- function(dataset_type, idx, method) {
  if (method %in% c("normal_ssd", "logis_ssd", "kde_ssd")) {
    ...
    if (file.exists(fname)) {
      ...
      compute_and_save(dataset_type, idx, method)
    }
  }
}
...
fit_all_methods_on_all_the_datasets <- function(dataset_list, funlist) {
  dataset_list %>%
    ...
    (function(grid) {
      mcmapply(FUN = save_if_not_present, grid$dataset_type, grid$idx, grid$method, mc.cores = detectCores() - 1)
    })
}
    ...
    lapply(FUN = function(list_of_fits) {
      list_of_fits %>%
        mclapply(FUN = function(fit) {
          fit %>%
            thin_fit(numit = numit) %>%
            get_quantiles_and_CI(p = 0.05, par_ = F, nbootsamples = nbootsamples)
        }, mc.cores = ncores) %>%
        Reduce(rbind, .) %>%
...
plot_average_density_estimates <- function(average_density_estimates, reference = NULL) {
  ...
  if (is.null(reference)) {
    ...
    p +
      geom_line(linetype = 2, linewidth = 1.5) +
      geom_line(data = refdens, aes(x = xs, y = dens), colour = "black", linewidth = 1.5)
  }
}
...
get_MISE_for_all_methods <- function(list_of_list_of_fits, reference_dist, numit = 10**6, ncores = 6) {
  list_of_list_of_fits %>%
    lapply(FUN = function(list_of_fits) {
      list_of_fits %>%
        mclapply(FUN = function(fit) {
          cmp_ISE_fit(fit %>% thin_fit(numit = numit), reference_dist)$value
        }, mc.cores = ncores) %>%
        unlist() %>%
  ...
  funlist %>%
    ...
    lapply(FUN = function(fun) {
      expand.grid(dataset_type, 1:length(dataset_list[[dataset_type %>% as.character()]]), fun, stringsAsFactors = F) %>%
        setNames(c("dataset_type", "idx", "method")) %>%
        (function(grid) {
          mapply(FUN = load_compute_or_pass, grid$dataset_type, grid$idx, grid$method, SIMPLIFY = F)
        }) %>%
        rmNull()
    })
}
...
expand.grid(c("normal", "student", "bimodal", "trimodal"), c(10, 20, 50, 100)) %>%
...
# Comparison on bimodal normal datasets

## Dataset of size 10

```{r}
xs_bimodal <- seq(-6, 10, length.out = 100)
```
...
fits_on_bimodal10_for_all_methods <- load_file("bimodal10")
```
...
```{r, eval=T}
fits_on_bimodal10_for_all_methods %>%
...
```{r, eval=T}
fits_on_bimodal10_for_all_methods %>%
  get_average_density_estimates(xs = xs_bimodal, ncores = 6) %>%
...
fits_on_bimodal20_for_all_methods <- load_file("bimodal20")
```
...
```{r, eval=T}
fits_on_bimodal20_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-2, 5), sigmas = c(1, 1), probs = c(2, 4) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
fits_on_bimodal50_for_all_methods <- load_file("bimodal50")
```
...
```{r, eval=T}
fits_on_bimodal50_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-2, 5), sigmas = c(1, 1), probs = c(2, 4) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
fits_on_bimodal100_for_all_methods <- load_file("bimodal100")
```
...
```{r, eval=T}
fits_on_bimodal100_for_all_methods %>%
...
# Comparison on trimodal normal datasets

## Dataset of size 10

```{r}
xs_trimodal <- seq(-6, 10, length.out = 100)
```
...
fits_on_trimodal10_for_all_methods <- load_file("trimodal10")
```

```{r, eval=T}
fits_on_trimodal10_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
```{r, eval=T}
fits_on_trimodal10_for_all_methods %>%
  get_MISE_for_all_methods(reference = function(x) dmixnorm(x, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()), numit = 2000, ncores = 6) %>%
  print_RMISE_for_all_methods()
```
...
```{r, eval=T}
fits_on_trimodal10_for_all_methods %>%
  get_average_density_estimates(xs = xs_trimodal, ncores = 6) %>%
  plot_average_density_estimates(function(x) dmixnorm(x, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()))
```
...
fits_on_trimodal20_for_all_methods <- load_file("trimodal20")
```

```{r, eval=T}
fits_on_trimodal20_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
fits_on_trimodal50_for_all_methods <- load_file("trimodal50")
```

```{r, eval=T}
fits_on_trimodal50_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
fits_on_trimodal100_for_all_methods <- load_file("trimodal100")
```
```{r, eval=T}
fits_on_trimodal100_for_all_methods %>%
  get_quantile_CI_mean_length_for_all_methods(reference_quantile = qmixnorm(p = 0.05, mus = c(-5, 0, 5), sigmas = c(1, 1, 2), probs = c(2, 3, 2) %>% normalise_weights()), numit = 10000, ncores = 6) %>%
  print_quantiles_for_all_methods()
```
...
fits_on_bimodal20_for_all_methods <- load_file("bimodal20")
xs_bimodal <- seq(-6, 10, length.out = 100)
average_density_estimates_bimodal20 <- fits_on_bimodal20_for_all_methods %>%
  get_average_density_estimates(xs = xs_bimodal)
```

```{r}
reference_grids <- list(xs_normal, xs_student, xs_bimodal) %>% setNames(c("normal", "student", "Multimodal"))

reference_densities <- list(
  "normal" = dnorm,
...
```{r}
average_density_estimates <- rbind(
  average_density_estimates_normal20 %>% mutate(data_type = "Normal"),
  average_density_estimates_student20 %>% mutate(data_type = "t-Student"),
  average_density_estimates_bimodal20 %>% mutate(data_type = "Multimodal")
) %>%
...
p <- average_density_estimates %>%
  ...
  geom_line(linewidth = 1.5, linetype = 2) +
