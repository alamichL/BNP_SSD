...
MixNRMI1dens_mvcpp = function (x, probs = c(0.025, 0.5, 0.975), Alpha = 1, Beta = 0,
  ...
  for (j in seq(Nit)) {
    ...
    ystar <- gs4(
      ystar, x, idx, distr.k = distr.k, sigma.k = sigma,
    ...
    y <-
      fcondYXA(
        x, distr = distr.k, Tau = Tau, J = J, sigma = sigma
      )
    sigma <- gs5(
      sigma, x, y, distr = distr.k, asigma = asigma,
...
environment(MixNRMI1dens_mvcpp) <- environment(MixNRMI1)
