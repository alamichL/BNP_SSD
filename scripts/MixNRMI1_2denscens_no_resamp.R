...
MixNRMI2denscens_no_resamp =  function (xleft, xright, probs = c(0.025, 0.5, 0.975), Alpha = 1, Beta = 0,
...
#     zstar <- tt$zstar
  ...
  return(
    list(
      ...
      means = means, sigmas = sigmas, weights = weights, Js
    )
  )
}

MixNRMI1denscens_no_resamp = function (xleft, xright, probs = c(0.025, 0.5, 0.975), Alpha = 1, Beta = 0,
...
environment(MixNRMI1denscens_no_resamp) <- environment(MixNRMI1)
...
environment(MixNRMI2denscens_no_resamp) <- environment(MixNRMI2)
# environment(MixNRMI2dens_no_resamp) <- environment(MixNRMI2)
# c('rk','dk','pk') %>%
#   lapply(FUN = function(x) unlockBinding(sym = x, environment(MixNRMI2denscens_mvcpp_faster)))
# assign(x = 'rk', value = rk, pos = environment(MixNRMI2denscens_mvcpp_faster))
# unlockBinding(sym = 'rk', environment(MixNRMI1denscens_mvcpp_faster))
# assign(x = 'rk', value = rk, pos = environment(MixNRMI1denscens_mvcpp_faster))
