...
convert_species_to_phylum <- function(keys) {
  keys %>%
    convert_to_major_taxon() %>%
    convert_major_to_phylum()
}
convert_species_to_SSDbookGroup <- function(keys) {
  keys %>%
    convert_to_major_taxon() %>%
    convert_major_to_SSDbookGroup()
}

plot_cdf_group_and_cluster <- function(fit, contaminant_short_name, optimal_clustering, species_convert_fun, title = TRUE, pointwise_CI = FALSE) {
  ...
  curve_dat <- tibble(xs = get_plotting_range(fit$data_)) %>%
    (function(df){
      if(pointwise_CI){
        get_CDF_with_quantiles_BNPdensity(fit, xs = df$xs) %>% 
          dplyr::rename(cdf_ = mean) %>% 
          return
      }
      else{
        df %>% 
    ...
    (function(df) {
      rbind(
        df %>% mutate(colour_scheme = "Coloured by species group"),
        df %>% mutate(colour_scheme = "Coloured by cluster membership")
      )
    })

  p <- tibble(
    loc_species_names = loc_species_names,
    ...
    (function(df) {
      rbind(
        df %>% mutate(colour_scheme = "Coloured by species group", colour = cv_nm),
  ...
  if(pointwise_CI){
    p = p + 
      geom_ribbon(data = curve_dat %>% 
                    mutate(colour_scheme = factor(colour_scheme, levels = c("Coloured by species group", "Coloured by cluster membership"))), 
                  aes(x = xs, y = NULL, ymin = lower, ymax = upper, colour = NULL, label = NULL), alpha = 0.2) +
      ylab("Fraction affected (with pointwise 95% CI)")
  }
...
print(p)
pdf("figures/example_clustering_post_analysis_credible_bands.pdf", width = 8, height = 4)
print(p)
dev.off()
```

```{r}
pl <- carbaryl_fit_nc$R %>%
  table() %>%
  (function(vv) {
    tibble(
      pk = vv %>% as.numeric(),
      k = names(vv) %>% as.numeric()
    )
  }) %>%
  ggplot(aes(x = k)) +
  theme_bw() +
  geom_segment(aes(xend = k, y = 0, yend = pk), linewidth = 1) +
  ggtitle("Posterior distribution of the number of clusters") +
  ylab("") +
  xlab("Number of clusters")

plot(pl)

ggsave("figures/posterior_number_of_clusters.pdf", pl, width = 4, height = 4)
```
